...
namespace GJ.DEV.COM
{
   ...
   public class CSerialPort
   {
      ...
      /// <returns></returns>
      public bool send(string wData, int rLen, out string rData, out string er, int timeOut = 1000)
      {
          er = string.Empty;
      ...
      public bool send(byte[] wBytes, int rLen, out byte[] rBytes, out string er, int timeOut = 500)
      {
          ...
          try
          {
              ...
              {
                  CTimer.WaitMs(2);
              } while ((_rs232.BytesToRead < rLen) && (Environment.TickCount - waitTime) < timeOut);
