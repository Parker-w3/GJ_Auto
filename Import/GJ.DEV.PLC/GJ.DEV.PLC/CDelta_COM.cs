      ...
      public bool Read(int plcAddr, ERegType regType, int startAddr, int startBin, out int rVal, out string er)
      {
          lock (_sync)
          {
              ...
              try
              {
                  ...
                  {
                     int rValue=System.Convert.ToInt32(rData.Substring( 2,2), 16) * 256 + System.Convert.ToInt32(rData.Substring(0, 2), 16);
                     int local = startAddr % 16;

                     rVal = (rValue & (1 << local )) == (1 << local ) ? 1 : 0;
          ...
          lock (_sync)
          {
              ...
              try
              {
                  ...
                  for (int i = 0; i < N; i++)
                  {
                      ...
                      {
                          temVal = System.Convert.ToInt32(rData.Substring(i * 4+2, 2), 16) * 256 + System.Convert.ToInt32(rData.Substring(i * 4 , 2), 16);
	    ...
	    {	        
            ...
            else
            {
                wr = 16;
                 ...
                 readAdrs = (wStartAddr / 16) * 16;
            }
       
            string wDate = wr.ToString("X4");

            if (!SendDeltaCommand(plcAddr, ":", wDataType, readAdrs, wLen, 2, wDate, out rData, out er))
                return false;
