...
namespace GJ.DEV.FCMB
{
    ...
    {
        #region 构造函数
        public CFMBThread(int idNo, string name, int startAddr, int endAddr, bool autoMode = false)
        {
            this._idNo = idNo;
        ...
        private bool readVersion(int addr, out string er)
        {
            ...
            try
            {
                ...
                _Mon[addr].Base.devVer = ((double)devVer / 10).ToString("0.0"); 
        ...
        /// <param name="er"></param>
        /// <returns></returns>
        private bool readChildVersion(int addr, out string er)
        {
            er = string.Empty;
            ...
            try
            {
                ...
                if (!_devFMB.ReadChildVersion(addr, out childVer, out er))
                {
                    Thread.Sleep(_delayMs);
        ...
        private bool readVolt(int addr, out string er)
        {
            ...
            try
            {
                ...
                if (!_devFMB.ReadVolt(addr, out Volt, out er))
                {
                    Thread.Sleep(_delayMs);
        ...
        private bool writeQCM(int addr, out string er)
        {
            ...
            try
            {
                ...
                if (!_devFMB.SetQCM(addr, _Mon[addr].Para.QCM.qcm,_Mon[addr].Para.QCM.qcv,_Mon[addr].Para.QCM.qci, out er))
                {
                    Thread.Sleep(_delayMs);
        ...
        private bool readTimer(int addr, out string er)
        {
            ...
            try
            {
                ...
                if (!_devFMB.ReadOnOffMode(addr, out mode, out er))
                {
                    ...
                    if (!_devFMB.ReadOnOffMode(addr, out mode, out er))
                    {
                        er = _Mon[addr].ToString() + "读ONOFF模式";
                        return false;
                    }
                }
        ...
        private bool writeACON(int addr, out string er)
        {
            ...
            try
            {
                ...
                if (!_devFMB.SetACON(addr, _Mon[addr].Para.ACON.wOnOff, out er))
                {
                    Thread.Sleep(_delayMs);
