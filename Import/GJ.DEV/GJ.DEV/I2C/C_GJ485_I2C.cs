...
namespace GJ.DEV.I2C
{
    public class C_GJ485_I2C
    {
        ...
        /// <param name="er"></param>
        /// <returns></returns>
        private bool Read(int devAddr, int regAddr, int PSUAdrs,int devAdrs, out int rValue, out string er)
        {
            rValue = 0;
            er = string.Empty;
            ...
            try
            {
                string wCmd = devAddr.ToString("X2");
                int rLen = 0;
                //01 10 80 FB 00 03 00 00 C4 00 BC 00 02 
                //01 10 80 FB 00 03 00 00 C4 00 BC 00 02 EC90
                wCmd += "10";      //寄存器功能码为03
                rLen = 2;

                wCmd += regAddr.ToString("X4");  //开始地址
                wCmd += "000300";                //数量+字节数
                wCmd += PSUAdrs.ToString("X4");  //PSU地址
                wCmd += devAdrs.ToString("X4");  //命令地址
                wCmd += rLen.ToString("X4");     //返回长度
               
                wCmd += Crc16(wCmd);                  //CRC16 低位前,高位后     
                string rData = string.Empty;
                if (!send(wCmd, 7 + rLen, out rData, out er))
                    return false;
                ...
                {
                    rData = temp.Substring(i * 4, 4) + rData;
                    rValue = Convert.ToInt16(rData, 16);
                }
  ...
  /// </summary>
  /// <param name="wAddr"></param>
  /// <param name="PSUAdrs"></param>
  /// <param name="devAdrs"></param>
  /// <param name="getData"></param>
  /// <param name="er"></param>
  /// <returns></returns>
        public bool GetTranValueData(int wAddr, int PSUAdrs, int devAdrs,  ref int getData, out string er)
        {
            er = string.Empty;
            ...
            try
            {
                int regAdrs = 0x80FB;
                string readData = string.Empty;
                if (!Read(wAddr, regAdrs,PSUAdrs,devAdrs ,out getData, out er))     //读取产品数据
                {
                    er = "读取IIC地址" + wAddr.ToString() + "产品数据失败__" + er;
                    return false;
                }
