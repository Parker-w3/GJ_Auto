    ...
    public class CDA_200_16 : ILED
    {
        ...
        public bool SetLoadValue(int wAddr, int chanNo, CLOAD chanPara, bool saveEEPROM, out string er)
        {
            ...
            try
            {
                ...
                if (chanPara.Von < 2)       //Von取值范围2-60V
                    chanPara.Von = 2;
                if (chanPara.Von >60)
                    chanPara.Von = 60;
                ...
                {
                    case EMODE.CC_Slow:
                        if (chanPara.load <0.5)
                            chanPara.load =0.5;
                        if (chanPara.load>20 )
                            chanPara.load = 20;
                        INFO += ((int)(chanPara.load * 100)).ToString("X4");    //CCSlow或CCFast，5/6字节值=电流（A）*100，取值范围0.5A～20A。     
                        INFO += ((int)(chanPara.mark * 10)).ToString("X2");     //当负载模式为CC Slow或者CC_Fast时，该参数的含义是负载启动延迟时间。第7字节值=延迟时间（秒）*10，取值范围0～20秒。
                        break;
                    case EMODE.CV:
                        if (chanPara.load <3)
                            chanPara.load =3;
                        if (chanPara.load>60 )
                            chanPara.load = 60;
                        INFO += ((int)(chanPara.load * 20)).ToString("X4");     //CV模式时，5/6字节值=电压（V）*20，取值范围3V～60V
                        INFO += "00";                                           //当负载模式为CV/CP/CR时，该参数无实际意义，需赋值为0。
                        break;
                    case EMODE.CP:
                        if (chanPara.load <2)
                            chanPara.load =2;
                        if (chanPara.load>320 )
                            chanPara.load = 320;
                        INFO += ((int)(chanPara.load * 10)).ToString("X4"); //负载值:CP模式时，设定值=功率（W）*10，取值范围2W～320W
                        INFO += "00";                                          //当负载模式为CV/CP/CR时，该参数无实际意义，需赋值为0。
                        break;
                    case EMODE.CR:
                        if (chanPara.load <2.4)
                            chanPara.load =2.4;
                        if (chanPara.load>4000 )
                            chanPara.load = 4000;
                    ...
                    case EMODE.CC_Fast:
                        ...
                        if (chanPara.load>20 )
                            chanPara.load = 20;
                        INFO += ((int)(chanPara.load * 1000)).ToString("X4"); ///CCSlow或CCFast，5/6字节值=电流（A）*100，取值范围0.5A～20A。    
                        INFO += ((int)(chanPara.mark * 10)).ToString("X2");   //当负载模式为CC Slow或者CC_Fast时，该参数的含义是负载启动延迟时间。第7字节值=延迟时间（秒）*10，取值范围0～20秒。
                        break;
                    default:
                        break;
        ...
        public bool SetLoadValue(int wAddr, List<CLOAD> chanList,bool saveEEPROM, out string er)
        {
            ...
            try
            {
                ...
                {
                    er = CLanguage.Lan("设置负载通道数量") + "[" + _maxCH.ToString() + "]"+ 
                         CLanguage.Lan("错误") + ":" + chanList.Count.ToString();
                    return false;
                }
                ...
                for (int i = 0; i < chanList.Count; i++)
                {
                    ...
                    if (chanList[i].Von < 2)       //Von取值范围2-60V
                        chanList[i].Von = 2;
                    if (chanList[i].Von > 60)
                        chanList[i].Von = 60;

                    INFO += ((int)(chanList[i].Von * 20)).ToString("X4"); //Von点-2Bytes

                    if (chanList[i].mark > 20)
                        chanList[i].mark = 20;
                    if (chanList[i].mark < 0)
                        chanList[i].mark = 0;

                    switch (chanList[i].Mode)
                    {
                        case EMODE.CC_Slow:
                            if (chanList[i].load < 0.5)
                                chanList[i].load = 0.5;
                            if (chanList[i].load > 20)
                                chanList[i].load = 20;
                            INFO += ((int)(chanList[i].load * 100)).ToString("X4");    //CCSlow或CCFast，5/6字节值=电流（A）*100，取值范围0.5A～20A。     
                            INFO += ((int)(chanList[i].mark * 10)).ToString("X2");     //当负载模式为CC Slow或者CC_Fast时，该参数的含义是负载启动延迟时间。第7字节值=延迟时间（秒）*10，取值范围0～20秒。
                            break;
                        case EMODE.CV:
                            if (chanList[i].load < 3)
                                chanList[i].load = 3;
                            if (chanList[i].load > 60)
                                chanList[i].load = 60;
                            INFO += ((int)(chanList[i].load * 20)).ToString("X4");     //CV模式时，5/6字节值=电压（V）*20，取值范围3V～60V
                            INFO += "00";                                           //当负载模式为CV/CP/CR时，该参数无实际意义，需赋值为0。
                            break;
                        case EMODE.CP:
                            if (chanList[i].load < 2)
                                chanList[i].load = 2;
                            if (chanList[i].load > 320)
                                chanList[i].load = 320;
                            INFO += ((int)(chanList[i].load * 10)).ToString("X4"); //负载值:CP模式时，设定值=功率（W）*10，取值范围2W～320W
                            INFO += "00";                                          //当负载模式为CV/CP/CR时，该参数无实际意义，需赋值为0。
                            break;
                        case EMODE.CR:
                            if (chanList[i].load < 2.4)
                                chanList[i].load = 2.4;
                            if (chanList[i].load > 4000)
                                chanList[i].load = 4000;
        ...
        public bool ReadLoadValue(int wAddr, ref CData data, out string er)
        {
            ...
            try
            {
                ...
                if (!checkErrorCode(rData, out rVal, out  temp))
                    return false;
                /*                 
                 * 第1-96字节为各个通道的实时数据，从第1通道开始，每个通道含6个字节数据，
                 * 每通道的第1/2字节为通道电压值、第3/4字节为通道的电流值、第5/6字节为通道状态信息。
                 * 第97字节数据为逆变器状态信息。
                 */
                ...
                for (int i = 0; i < _maxCH; i++)
                {
                    ...
                    int byte2 = System.Convert.ToInt16(rCmd.Substring(8, 4), 16); //通道状态信息

                    CCHData ch = new CCHData();

                    ch.volt = (double)byte0 / 20;   //电压（V）=2byte值 /20

                    ch.current = (double)byte1 / 100; //电流（A）=2byte值 /100
