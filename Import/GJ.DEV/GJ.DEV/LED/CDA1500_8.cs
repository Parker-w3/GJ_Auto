...
{
    public class CDA1500_8 : ILED
    {
        #region 构造函数
        public CDA1500_8(int idNo, string name)
        {
            this._idNo = idNo;
            this._name = name;
        }
        ...
        public bool SetNewAddr(int wAddr, out string er)
        {
            ...
            try
            {
                ...
                {
                    er = "设置地址范围" + "[1-127]:" + wAddr.ToString();
                    return false;
                }
                string CID = "01";
                string INFO = wAddr.ToString("X2");
                string wCmd = CalDataForCmd(0, CID, INFO);
                string rData = string.Empty;
                int rLen = C_LEN;
                if (!SendCmd(wCmd, rLen, out rData, out er))
                    return false;
                return true;
            }
            catch (Exception ex)
        ...
        public bool ReadVersion(int wAddr, out string version, out string er)
        {
            ...
            try
            {
                ...
                string wCmd = CalDataForCmd(wAddr, CID, INFO);
                string rData = string.Empty;
                int rLen = C_LEN + 2;
                if (!SendCmd(wCmd, rLen, out rData, out er))
                    return false;
            ...
            try
            {
                ...
                */
                ...
                for (int i = 0; i < _maxCH; i++)
                {
                    ...
                    {
                        case EMODE.CC_Slow:
                            para.load = (double)byte2 / 1000; //CCSlow或CCFast或者MTK模式:读值=电流（A）*100，取值范围0.1A～60A。
                            para.unit = "A";
        ...
        public bool ReadLoadValue(int wAddr, ref CData data, out string er)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _maxCH; i++)
                {
                    ...
                    CHAN_DATA ch = new CHAN_DATA();

                    ch.volt = (double)byte0 / 20;   //电压（V）=2byte值 /20

                    ch.current = (double)byte1 / 100; //电流（A）=2byte值 /1000
            ...
            try
            {
                ...
                switch (chanPara.Mode)
                {
                    case EMODE.CC_Slow:
                        ...
                        INFO += 0.ToString("X2");  //负载模式-1Bytes

                        INFO += ((int)(chanPara.Von * 20)).ToString("X4"); //Von点-2Bytes

                        INFO += ((int)(chanPara.load * 1000)).ToString("X4"); //负载值:CCSlow或CCFast或者MTK模式时，设定值=电流（A）*100，取值范围0.1A～60A
                        INFO += ((int)(chanPara.mark * 10)).ToString("X2");   //模式附加参数:负载启动延迟时间。设定值=延迟时间（秒）*10，取值范围0～20秒
                        break;
                    case EMODE.CV:
                        INFO += 1.ToString("X2");  //负载模式-1Bytes
                    ...
                    case EMODE.CP:
                        ...
                        INFO += 2.ToString("X2");  //负载模式-1Bytes
                        ...
                        break;
                    case EMODE.CR:
                        INFO += 3.ToString("X2");  //负载模式-1Bytes
                    ...
                    case EMODE.CC_Fast:
                        ...
                        INFO += 4.ToString("X2");  //负载模式-1Bytes
                        ...
                        break;
                    case EMODE.LED_Slow:
                        INFO += 5.ToString("X2");  //负载模式-1Bytes
                        ...
                        break;
                    case EMODE.LED_Fast:
                        INFO += 6.ToString("X2");  //负载模式-1Bytes
                    ...
                    case EMODE.Power:
                        ...
                        INFO += 7.ToString("X2");  //负载模式-1Bytes

                        INFO += ((int)(chanPara.Von * 20)).ToString("X4"); //Von点-2Bytes

                        INFO += ((int)(chanPara.load * 20)).ToString("X4"); //负载值:LED模式时，设定值=电压（V）*20，取值范围12V～400V
                        INFO += ((int)(chanPara.mark)).ToString("X2");   //模式附加参数:当负载模式为LED模式时，该参数为LED电流。设定值=电流（A）*10，取值范围：0.1A～5A
                        break;
                    default:
                        break;
                }
        ...
        public bool SetLoadValue(int wAddr, List<CLOAD> chanList, bool saveEEPROM, out string er)
        {
            ...
            try
            {
                ...
                //{
                //    er = ("设置负载通道数量") + "[" + _maxCH.ToString() + "]" + ("错误") + ":" + chanList.Count.ToString();
                //    return false;
                //}
                ...
                for (int i = 0; i < chanList.Count; i++)
                {
                    ...
                    switch (chanList[i].Mode)
                    {
                        case EMODE.CC_Slow:
                            ...
                            INFO += ((int)(chanList[i].load * 1000)).ToString("X4"); //负载值:CCSlow或CCFast或者MTK模式时，设定值=电流（A）*100，取值范围0.1A～60A
      ...
      */
        ...
        /// <returns></returns>
        private bool SendCmd(string wData, int rLen, out string rData, out string er, int wTimeOut =300)
        {
            rData = string.Empty;
            er = string.Empty;
            try
        ...
        /// <returns></returns>
        private string CalDataForCmd(int ADR, string CID, string INFO)
        {
            try
            {
        ...
        private bool ERS_CheckSum(string rData, out string er)
        {
            ...
            try
            {
                ...
                {
                    er = "数据长度小于" + C_LEN.ToString() + ":" + rData;
                    return false;
                }
        ...
        private string CalCheckSum(string wCmd)
        {
            try
            {
                ...
                checkSum = chk_H.Replace(EOI, ROI) + chk_L.Replace(EOI, ROI);
                return checkSum;
            }
            catch (Exception)
            {
