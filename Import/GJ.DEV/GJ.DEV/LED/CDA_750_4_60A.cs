...
{
    public class CDA_750_4_60A: ILED
    {
        #region 构造函数
        public CDA_750_4_60A(int idNo, string name)
        {
            this._idNo = idNo;
            this._name = name;
        }
            ...
            try
            {
                ...
                */
                ...
                for (int i = 0; i < _maxCH; i++)
                {
                    ...
                    {
                        case EMODE.CC_Slow:
                            para.load = (double)byte2 / 100; //CCSlow或CCFast或者MTK模式:读值=电流（A）*100，取值范围0.1A～60A。
                            para.unit = "A";
        ...
        public bool SetLoadValue(int wAddr, List<CLOAD> chanList, bool saveEEPROM, out string er)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < chanList.Count; i++)
                {
                    ...
                    {
                        case EMODE.CC_Slow:
                            INFO += ((int)(chanList[i].load * 100)).ToString("X4"); //负载值:CCSlow或CCFast或者MTK模式时，设定值=电流（A）*100，取值范围0.1A～60A
