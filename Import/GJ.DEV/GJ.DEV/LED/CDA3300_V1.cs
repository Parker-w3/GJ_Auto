...
{
    public class CDA3300_V1 : ILED
    {
        #region 构造函数
        public CDA3300_V1(int idNo, string name)
        {
            this._idNo = idNo;
            this._name = name;
        }
        ...
        public bool SetLoadValue(int wAddr, int chanNo, CLOAD chanPara, bool saveEEPROM, out string er)
        {
            ...
            try
            {
                ...
                {
                    case EMODE.CC_Slow:
                        INFO += ((int)(chanPara.load * 100)).ToString("X4"); //负载值:CCSlow或CCFast或者MTK模式时，设定值=电流（A）*100，取值范围0.1A～60A
