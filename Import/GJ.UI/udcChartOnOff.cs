...
namespace GJ.UI
{
    ...
    public partial class udcChartOnOff : UserControl
    {
        ...
        private void refreshUI()
        {
            ...
            do
            {
                for (int i = 0; i < _onoff.Count; i++)
                {
                    ...
                    {
                        calOnOffWaveing(_onoff[i].curVolt, _onoff[i].onoffTimes, _onoff[i].onTimes, _onoff[i].offTimes);
                        runTime -= _onoff[i].onoffTimes;
                    }
                    else
                    {
                        calOnOffWaveing(_onoff[i].curVolt, runTime, _onoff[i].onTimes, _onoff[i].offTimes);
                        runTime = 0;
                    }
                    if (runTime < 0)
                        runTime = 0;
        ...
        /// 更新重画曲线
        /// </summary>
        public void RefOnOffUI(int runTime)
        {
            try
            {
                runningTime = runTime;
