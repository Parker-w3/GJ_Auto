    ...
    public partial class FrmDD : Form,IChildMsg
    {
        ...
        private void btnOpen_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (comMon == null)
                {
                    ...
                    {
                        labStatus.Text = CLanguage.Lan( "找不到【") + cmbDDType.Text + CLanguage.Lan( "】类型");
                        labStatus.ForeColor = Color.Red;
                        return;
                    }
        ...
        private void btnSetAddr_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (comMon.SetNewAddr(addr, out er))
                    ...
                    showInfo(CLanguage.Lan( "设置当前地址[") + addr.ToString("D2") + CLanguage.Lan( "]失败:") + er, true);
            }
            catch (Exception ex)
            {
        ...
        private void btnSetA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (comMon.SetNewLoad(addr,para, out er))
                    showInfo(CLanguage.Lan( "成功设置负载电流[") + addr.ToString("D2") + "]");
                else
                    showInfo(CLanguage.Lan( "设置当前负载电流[") + addr.ToString("D2") + CLanguage.Lan( "]失败:") + er, true);
        ...
        private void btnReadA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                  showInfo(CLanguage.Lan( "读取电流设置[") + addr.ToString("D2") + CLanguage.Lan( "]失败:") + er, true);
                  return;
                }
                ...
                showInfo(CLanguage.Lan( "成功读取电流设置[") + addr.ToString("D2") + "]");
        ...
        private void btnReadData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    showInfo(CLanguage.Lan( "读取DD模块数据[") + addr.ToString("D2") + CLanguage.Lan( "]失败:") + er, true);
                    return;
                }
                ...
                showInfo(CLanguage.Lan( "成功读取DD模块数据[") + addr.ToString("D2") + "]");
        ...
        private void SetLanguage()
        {
            ...
            switch (CLanguage.languageType)
            {
                ...
                case CLanguage.EL.繁体:
                    this.Text = CLanguage.Lan("DD模块调试工具");
                    btnOpen.Text = CLanguage.Lan("打开");
                    btnReadA.Text = CLanguage.Lan("回读电流");
                    btnReadData.Text = CLanguage.Lan("读取数据");
                    btnScan.Text = CLanguage.Lan("开始扫描");
                    btnSetA.Text = CLanguage.Lan("设定电流");
                    btnSetAddr.Text = CLanguage.Lan("设置地址");
                    Column1.HeaderText = CLanguage.Lan("地址");
                    Column2.HeaderText = CLanguage.Lan("结果");
                    Column4.HeaderText = CLanguage.Lan("读值");
                    label1.Text = CLanguage.Lan("模块类型:");
                    label10.Text = CLanguage.Lan("电流下限(A):");
                    label11.Text = CLanguage.Lan("电流上限(A):");
                    label12.Text = CLanguage.Lan("电压读值(V)");
                    label13.Text = CLanguage.Lan("开始地址:");
                    label14.Text = CLanguage.Lan("结束地址:");
                    label2.Text = CLanguage.Lan("同步信号:");
                    label29.Text = CLanguage.Lan("串口号:");
                    label3.Text = CLanguage.Lan("模块地址:");
                    label4.Text = CLanguage.Lan("电压下限(V):");
                    label5.Text = CLanguage.Lan("电压上限(V):");
                    label6.Text = CLanguage.Lan("通道");
                    label7.Text = CLanguage.Lan("设定值(A)");
                    label8.Text = CLanguage.Lan("电流读值(A)");
                    label9.Text = CLanguage.Lan("状态指示:");
                    break;
                default:
                    break;
            }
