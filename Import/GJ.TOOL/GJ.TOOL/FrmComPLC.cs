...
namespace GJ.TOOL
{
    public partial class FrmComPLC : Form, IChildMsg
    {
        ...
        private void FrmComPLC_Load(object sender, EventArgs e)
        {
            ...
            cmbDataType.Items.Add("10进制数值");
            cmbDataType.Items.Add("16进制字符");
        ...
        private void cmbPLCType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(CLanguage.Lan("找不到对应[") + strType + CLanguage.Lan("]程序集"), CLanguage.Lan("错误"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            if (strType.EndsWith("TCP") || strType.EndsWith("UDP"))
            {
                ...
                {
                    txtAddr.Text = "192.168.3.200";
                    labBaud.Text = CLanguage.Lan("PLC端口:");
                    txtBaud.Text = "502";
                }
                else
                {
                    txtAddr.Text = "192.168.250.1";
                    labBaud.Text = CLanguage.Lan("PLC端口:");
                    txtBaud.Text = "9600";
                }
            }
        ...
        private void btnW_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(ex.ToString(), CLanguage.Lan("错误"), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                btnW.Enabled = true;
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!loadDefault)
                {
                    MessageBox.Show(CLanguage.Lan("正在初始化PLC寄存器,请稍等."), CLanguage.Lan("初始化"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                {
                    MessageBox.Show(er, CLanguage.Lan("错误提示"), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        private void btnRun_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (btnRun.Text == CLanguage.Lan("启动"))
                {
                    ...
                    {
                        MessageBox.Show(CLanguage.Lan("没有可监控寄存器,"), CLanguage.Lan("监控寄存器"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
        ...
        private void onBtn_Click(object sender, EventArgs e)
        {
            ...
            if (PLCThread == null)
            {
                MessageBox.Show(CLanguage.Lan("请先启动监控,"), CLanguage.Lan("监控寄存器"), MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            if (idNo == -1)
            {
                MessageBox.Show(CLanguage.Lan("寄存器[") + des + CLanguage.Lan("]不存在"), CLanguage.Lan("监控寄存器"),
        ...
        private void SetLanguage()
        {
            ...
            switch (CLanguage.languageType)
            {
                ...
                case CLanguage.EL.繁体:
                    this.Text = CLanguage.Lan("PLC通用调试工具");
                    btnCon.Text = CLanguage.Lan("连接");
                    btnLoad.Text = CLanguage.Lan("加载");
                    btnR.Text = CLanguage.Lan("读取");
                    btnRun.Text = CLanguage.Lan("启动");
                    btnW.Text = CLanguage.Lan("写入");
                    labBaud.Text = CLanguage.Lan("串口波特率:");
                    label1.Text = CLanguage.Lan("串口编号:");
                    label10.Text = CLanguage.Lan("读数值:");
                    label11.Text = CLanguage.Lan("通信时间:");
                    label2.Text = CLanguage.Lan("状态指示:");
                    label29.Text = CLanguage.Lan("PLC类型:");
                    label3.Text = CLanguage.Lan("IO监控数据库路径:");
                    label30.Text = CLanguage.Lan("位地址:");
                    label33.Text = CLanguage.Lan("数据类型:");
                    label4.Text = CLanguage.Lan("PLC地址:");
                    label5.Text = CLanguage.Lan("地址类型:");
                    label6.Text = CLanguage.Lan("开始地址:");
                    label7.Text = CLanguage.Lan("地址长度:");
                    label8.Text = CLanguage.Lan("写数据:");
                    label9.Text = CLanguage.Lan("读字符:");
                    labT1.Text = CLanguage.Lan("数值:低位在前,高位在后");
                    labT2.Text = CLanguage.Lan("字符:高位在前,低位在后");
                    break;
                default:
                    break;
            }
