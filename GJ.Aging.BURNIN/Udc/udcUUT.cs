...
{
    public partial class udcUUT : UserControl
    {
        #region 枚举
        /// <summary>
        /// UI状态
        ...
        public udcUUT(CUUT runUUT, int FixNo, int idNo)
        {
            InitializeComponent();
        ...
        private void tlClearFail_Click(object sender, EventArgs e)
        {
            ...
            {
                menuClick.OnEvented(new CSetMenuArgs(_iTimer, _idNo, ESetMenu.设定误测));
            }

        }

        private void tlClearAllFail_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(CLanguage.Lan("确定要设定所有位置为误测?"), CLanguage.Lan("状态设置"), MessageBoxButtons.YesNo,
                MessageBoxIcon.Question) == DialogResult.Yes)
            {
                menuClick.OnEvented(new CSetMenuArgs(_iTimer, _idNo, ESetMenu.整区误测));
            }
        }
        private void tlSetNA_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(CLanguage.Lan("确定要将该位置设定为无机?"), CLanguage.Lan("状态设置"), MessageBoxButtons.YesNo,
                MessageBoxIcon.Question) == DialogResult.Yes)
            {
                menuClick.OnEvented(new CSetMenuArgs(_iTimer, _idNo, ESetMenu.设定无机));
            }
        }
        private void tlRefAlarm_Click(object sender, EventArgs e)
        {
            menuClick.OnEvented(new CSetMenuArgs(_iTimer, _idNo, ESetMenu.复位报警));
         }

        private void tlChooseModel_Click(object sender, EventArgs e)
        {
            menuClick.OnEvented(new CSetMenuArgs(_iTimer, _idNo, ESetMenu.选择机种));
        }
        #endregion
        ...
        private void setBI(int idNO)
        {
            ...
            for (int i = 0; i < _uutMax; i++)
            {
                ...
                bool havebar = false;
                if (_runUUT.Led[isolt].barType == 1 && _runUUT.Led[isolt].serialNo != string.Empty)
                    havebar = true;
                ...
                if (_runUUT.Led[isolt].result != 0)
                {
                    ...
                    //else
                    //    result = 1;
                    result = _runUUT.Led[isolt].result;
                }
                ...
                if (haveUUT == true)
                {
                    ...
                    {
                        eloadmsg = "负载串口:" + CGlobalPara.SysPara.Dev.GJ_1050_4Com[1];
                    }
                    else
                    {
                ...
                if (havebar)
                    lblUUT[i].ForeColor = Color.Blue;
                else
                    lblUUT[i].ForeColor = Color.Black;
                switch (result)
                {
                    case 0:
                    ...
                    default:
                        ...
                        break;
                        //case 3:
                        //    lblUUT[i].BackColor = Color.Blue  ;
                        //    break;
                        //case 4:
                        //    lblUUT[i].BackColor = Color.Yellow ;
                        //    break;
                        //case 5:
                        //    lblUUT[i].BackColor = Color.Pink ;
                        //    break;
                        //case 6:
                        //    lblUUT[i].BackColor = Color.LightCoral;
                        //    break;
                        //case 7:
                        //    lblUUT[i].BackColor = Color.LightGray;
                        //    break;
                        //default:
                        //    lblUUT[i].BackColor = Color.WhiteSmoke;
                        //    break;
